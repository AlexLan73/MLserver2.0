        ...
        public void RunA()
        {
            for (int n = 0; n <= _ncount; n++)
            {
                ...
                if (n % 2 == 0)
                {
                    ...
                    var _x01 = Task<bool>.Run(() => p(new Guid(idGuid.ToByteArray()), n));
                    ThreadManager.AddA(new Guid(idGuid.ToByteArray()), _x01, " _o12__ " + n.ToString());
                ...
                else
                {
                    Action<Guid, int> p1 = (__idGuid, i) =>
                    {
                        ...
                        ThreadManager.AddA(_idG, SetFalse, " _o21_ " + i.ToString());
                        for (int i0 = 0; i0 < 100; i0++)
                        {
                            Thread.Sleep(200);
                        }
                        ThreadManager.DelXXA(_idG);
                    };
                    var _x02 = Task<bool>.Run(() => p1(new Guid(idGuid.ToByteArray()), n));
                    ThreadManager.AddA(new Guid(idGuid.ToByteArray()), _x02, " _o22__ " + n.ToString());
                }
                ThreadManager.FindNotRunTaskA();
            }

            Thread.Sleep(1000);
            List<Guid> _lcount = ThreadManager.FindNotRunTaskA();
            bool _iscount = _lcount.Count > 0;
            int kk = 0;
            while (_iscount)
            {
                ...
                Thread.Sleep(500);
                _lcount = ThreadManager.FindNotRunTaskA();
                _iscount = _lcount.Count > 0;

                kk++;
                if (kk > 20 && _lcount.Count > 0)
                {
                    ThreadManager.DelXA(_lcount[0]);
                }
            }
        }
    }
